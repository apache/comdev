...
function resultPage(success, message) {
  ...
  const autoClose = success
    ? `<script>setTimeout(() => { document.title = "Done"; window.close(); }, 3000);</script>`
    : "";
  return `<!DOCTYPE html>
<html>
<head><title>PonyMail MCP ${success ? "Success" : "Error"}</title></head>
<body style="font-family:system-ui;max-width:640px;margin:80px auto;padding:0 20px;text-align:center">
  <h1 style="color:${color}">${icon} ${message}</h1>
  ${success ? "<p>You can close this tab. The MCP server is now authenticated.</p>" : '<p><a href="/">← Try again</a></p>'}
  ${autoClose}
</body>
</html>`;
}
